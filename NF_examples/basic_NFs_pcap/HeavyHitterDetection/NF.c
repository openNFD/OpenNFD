...
int _t9=1;
int _t10=1;
State<unordered_map<IP,int>> hh(*(new unordered_map<IP,int>()));
State<unordered_map<IP,int>> hh_counter(*(new unordered_map<IP,int>()));
State<int> threshold(_t1);
int process(Flow &f){

if (((*(int*) f.headers[FLAG_SYN]) == _t2)&&(hh[f][(*(IP*) f.headers[SIP])] != _t3 && hh_counter[f][(*(IP*) f.headers[SIP])] != threshold[f])){
hh_counter[f][(*(IP*) f.headers[SIP])]=hh_counter[f][(*(IP*) f.headers[SIP])] + _t4;

}
else if (((*(int*) f.headers[FLAG_SYN]) == _t5)&&(hh[f][(*(IP*) f.headers[SIP])] != _t6 && hh_counter[f][(*(IP*) f.headers[SIP])] == threshold[f])){
hh[f][(*(IP*) f.headers[SIP])]=_t7;

}
else if (((*(int*) f.headers[FLAG_SYN]) == _t8)&&(hh[f][(*(IP*) f.headers[SIP])] == _t9)){
}
else if ((*(int*) f.headers[FLAG_SYN]) != _t10){
}
        f.clean();
        return 0;
}
