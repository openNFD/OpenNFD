...
Flow::Flow(int* tag){
	...
	this->headers[Iplen] = new int(tag);
...
this->pkt = packet;
    ...
    u_char * ip_header =  packet+ethernet_header_length;
    IPHdr * ip_hdr = (IPHdr*) (ip_header);
    int src_addr = ntohl(ip_hdr->ip_src.s_addr);
	this->headers[Sip] = new IP(src_addr, 32);
	//this->field_value["sip"] = this->pkt;
    ...
    this->headers[Sport] = new int(tcph->th_sport);
    this->headers[Dport] = new int(tcph->th_dport);
}
void Flow::clean() {	
}
