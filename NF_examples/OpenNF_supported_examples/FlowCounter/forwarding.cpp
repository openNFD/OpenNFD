...
void stop(int signo)
{
	...
	printf("\n\n**************************************************\n");
	printf("%ld are forwarded ! Average Tput is %f \n",config.fw_sta.packetCount, config.fw_sta.packetCount / total);
	printf("counter = %ld   begin = %ld  end = %ld \n", config.fw_sta.packetCount,begin_t ,end_t);
	printf("**************************************************\n\n");
	exit(0);
}

State<int> packet_counter(*(new int(0)), "sip&dip");
int PacketCount(u_char * pkt) {
    Flow f(pkt);
    if (1){
    	packet_counter[f] = packet_counter[f] + 1;
        //printf("sip: %d, dip: %d   counter: %d\n",((IP*)f["sip"])->ip,((IP*)f["dip"])->ip, packet_counter[f]);
    }
...
int local_get_perflow(PerflowKey* key, int id, int raiseEvents,SDMBNExt *t){
	...
	if (key->wildcards & WILDCARD_NW_SRC){
		...
		unordered_map<Tuple, int> allState = packet_counter.getAllState();
...
int local_put_perflow(int id, PerflowKey * key, char *state){
	 ...
	 mul_s* tem = (mul_s*)ser_parse(head_tra,"mul_s",state,NULL);
	 	...
	 	if(tem==NULL){
	 		...
	 		packet_counter.addMultiState(tp,tem->count);
...
int local_del_perflow(PerflowKey *key,int id){
	...
	if (NULL == key)
	    { return -1; }
	int count = packet_counter.getAllState().size();

	pthread_mutex_lock(&counterLock);

		packet_counter.clean();
...
//					printf("%s \n",state);
		...
		//to build the tuple
			...
			///build the mul_state
			tem->tp = tmp;
			tem->count = packet_counter.getMultistate(tp);
			tem->next = NULL;
